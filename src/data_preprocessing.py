...
if __name__ == "__main__":
    ...
    emails_df = load_data(PROJECT_ROOT / 'data' / 'emails.csv')
    print("Columns in emails.csv:", emails_df.columns)

    # Load and check columns of email.csv if present
    email_csv_path = PROJECT_ROOT / 'data' / 'email.csv'
    if email_csv_path.exists():
        email_df = load_data(email_csv_path)
    ...
    emails_df.to_csv(PROJECT_ROOT / 'data' / 'preprocessed_emails.csv', index=False)
